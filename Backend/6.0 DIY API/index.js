...
app.put("/jokes/:id", (req, res) => {
  ...
  const searchInd = jokes.find((joke) => joke.id === id);
  jokes[searchInd] = replaceJoke;
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const editedInd = jokes.findIndex((joke) => joke.id === id);
  const patchJoke = {
    id: id,
    type: req.body.type || jokes[editedInd].type,
    text: req.body.text || jokes[editedInd].text,
  };
  jokes[editedInd] = patchJoke;
  console.log(jokes[editedInd]);
  res.json(patchJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const searchInd = jokes.findIndex((joke) => joke.id === id);
  jokes.splice(searchInd, 1);
  res.sendStatus(200);
});
